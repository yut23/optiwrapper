...
import ctypes
from ctypes import (POINTER, Structure, c_char_p, c_int, c_long, c_uint, c_ulong, c_void_p)
import sys
from typing import Callable, Iterable, Optional, List

from Xlib import X, display, error  # type: ignore

myxdo = ctypes.CDLL('/home/eric/Games/wrapper/myxdo.so')
...
def _errcheck_cb(result, func, arguments):
    ...
    if result != 0:
        raise XdoException(
            result,
            'Function {0} returned error code {1}'.format(func.__name__, result))
...
def search_windows(xdo: xdo_t,
    ...
    myxdo.xdo_search_windows(xdo, search, ctypes.byref(windowlist_ret),
                             ctypes.byref(nwindows_ret))

    return [windowlist_ret[i] for i in range(nwindows_ret.value)]

def watch_focus(window_ids: Iterable[int],
                focus_in_cb: Callable[[display.event.FocusIn], None],
                focus_out_cb: Callable[[display.event.FocusOut], None]) -> int:
    """
    ...
    while True:
        ...
        if evt.type == X.FocusOut and focused in (evt.window.id, -1) and evt.detail != X.NotifyInferior:
            focus_out_cb(evt)
            focused = 0
...
if __name__ == '__main__':
    ...
    def focus_in(evt: display.event.FocusIn) -> None:
        ...
        print('Got  focus on window 0x{:07x} ({:s}) "{:s}"'.format(
    ...
    def focus_out(evt: display.event.FocusOut) -> None:
        ...
        """
        cls = evt.window.get_wm_class() or ('', '')
        print('Lost focus on window 0x{:07x} ({:s}) "{:s}"'.format(
            evt.window.id, DETAIL_LUT[evt.detail], cls[1]))
